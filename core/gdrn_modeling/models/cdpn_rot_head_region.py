...
class RotWithRegionHead(nn.Module):
    def __init__(
        ...
        rot_output_dim=3,
        mask_output_dim=1,
        depth_output_dim=1, # zty #TODO
        freeze=False,
        num_classes=1,
        rot_class_aware=False,
        mask_class_aware=False,
        region_class_aware=False,
        depth_class_aware=False, # zty #TODO
        num_regions=64,
        norm="BN",
        num_gn_groups=32,
    ):
        ...
        self.depth_output_dim = depth_output_dim # zty #TODO
        if depth_class_aware:
            self.depth_output_dim *= num_classes


        self.features.append(
            nn.Conv2d(
                num_filters,
                self.mask_output_dim + self.rot_output_dim + self.region_output_dim + self.depth_output_dim, # zty #TODO
        ...
        if self.concat:
            ...
            if self.freeze:
                with torch.no_grad():
                    ...
                    depth = x[:, self.mask_output_dim + self.rot_output_dim + self.region_output_dim :, :, :] # zty #TODO
                    bs, c, h, w = norm.shape
                    ...
                    norm_dyn_y = norm[:, 4, :, :, :]
                    norm_dyn_z = norm[:, 5, :, :, :]
                    return (mask.detach(), norm_stc_x.detach(), norm_stc_y.detach(), norm_stc_z.detach(), norm_dyn_x.detach(), norm_dyn_y.detach(), norm_dyn_z.detach(), region.detach(), depth.detach()) # zty #TODO
            else:
                for i, l in enumerate(self.features):
                    x = l(x)
                mask = x[:, : self.mask_output_dim, :, :]
                ...
                norm_dyn_y = norm[:, 4, :, :, :]
                norm_dyn_z = norm[:, 5, :, :, :]
                return mask, norm_stc_x, norm_stc_y, norm_stc_z, norm_dyn_x, norm_dyn_y, norm_dyn_z, region, depth # zty #TODO
