...
class YCBV_PBR_Dataset:
    def __init__(self, data_cfg):
        ...
        self.norm_root = self.xyz_root.replace('xyz_crop', 'norm_crop')
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    stc_norm_path = osp.join(self.norm_root, f"{scene_id:06d}/{int_im_id:06d}_{anno_i:06d}-normal-static.pkl")
                    assert osp.exists(stc_norm_path), stc_norm_path
                    
                    dyn_norm_path = osp.join(self.norm_root, f"{scene_id:06d}/{int_im_id:06d}_{anno_i:06d}-normal-dynamic-gl.pkl")
