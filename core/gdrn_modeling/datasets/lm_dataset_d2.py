...
class LM_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.norm_prefixes = data_cfg["norm_prefixes"]
        self.depth_prefixes = data_cfg["depth_prefixes"] #TODO: zty
    ...
    def __call__(self):  # LM_Dataset
        ...
        assert len(self.ann_files) == len(self.norm_prefixes), f"{len(self.ann_files)} != {len(self.norm_prefixes)}"
        for ann_file, scene_root, xyz_root, norm_root, depth_render_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes, self.norm_prefixes, self.depth_prefixes):
            # linemod each scene is an object
            # scene_root, xyz_root, norm_root = scene_root.replace("\\", "/"), xyz_root.replace("\\", "/"), norm_root.replace("\\", "/")
            ...
            for im_id in tqdm(indices):
                ...
                # load render depth
                # depth_path = osp.join(scene_root,"depth_render/{:06d}.npz".format(int_im_id)) #TODO: zty
                ...
                record = {
                    ...
                    "file_name": osp.abspath(rgb_path),
                    # "depth_file": osp.relpath(depth_path, PROJ_ROOT),
                    "depth_file": osp.abspath(depth_path),
                    "height": self.height,
                    "width": self.width,
                    "image_id": int_im_id,
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    if "test" not in self.name:
                        ...
                        stc_norm_path = osp.join(norm_root, f"{int_im_id:06d}-normal-static.pkl")
                        assert osp.exists(stc_norm_path), stc_norm_path
                        inst["stc_norm_path"] = stc_norm_path

                        dyn_norm_path = osp.join(norm_root, f"{int_im_id:06d}-normal-dynamic-gl.pkl")
                        assert osp.exists(dyn_norm_path), dyn_norm_path
                        inst["dyn_norm_path"] = dyn_norm_path

                        depth_render_path = osp.join(depth_render_root, f"{int_im_id:06d}.npz")
                        assert osp.exists(depth_render_path), depth_render_path
                        inst["depth_render_path"] = depth_render_path
...
SPLITS_LM = dict(
    lm_13_train=dict(
        ...
        norm_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/norm_crop/{:06d}".format(ref.lm_full.obj2id[_obj]))
            for _obj in LM_13_OBJECTS
        ],
        depth_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/depth_render/{:06d}/depth_render".format(ref.lm_full.obj2id[_obj]))
            for _obj in LM_13_OBJECTS
        ],
    ...
    lmo_train=dict(
        ...
        norm_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/norm_crop/{:06d}".format(ref.lmo_full.obj2id[_obj]))
            for _obj in LM_OCC_OBJECTS
        ],
        depth_prefixes=[
            osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/depth_render/{:06d}/depth_render".format(ref.lmo_full.obj2id[_obj]))
            for _obj in LM_OCC_OBJECTS
        ],
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
    ...
    for split in ["train", "test", "all"]:
        ...
        if name not in SPLITS_LM:
            SPLITS_LM[name] = dict(
                ...
                norm_prefixes=[
                    osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/norm_crop/{:06d}".format(ref.lm_full.obj2id[obj]))
                ],
                depth_prefixes=[
                    osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/depth_render/{:06d}/render".format(ref.lm_full.obj2id[obj]))
                ],
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
                with_depth=True,  # (load depth path here, but may not use it)
        ...
        for debug_im_id in debug_im_ids[split][obj]:
            ...
            if name not in SPLITS_LM:
                SPLITS_LM[name] = dict(
                    ...
                    depth_prefixes=[
                        osp.join(DATASETS_ROOT, "BOP_DATASETS/lm/test/depth_render/{:06d}/depth_render".format(ref.lm_full.obj2id[obj]))
                    ],
                    scale_to_meter=0.001,
                    with_masks=True,  # (load masks but may not use it)
                    with_depth=True,  # (load depth path here, but may not use it)
