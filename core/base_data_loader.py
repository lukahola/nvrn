...
class Base_DatasetFromList(data.Dataset):
    ...
    def replace_bg(self, im, im_mask, return_mask=False):
        ...
        # logger.info(f'read bg image {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        # logger.info(f'truncate fg image {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        # copy fg might faster
        bg_img[mask] = im[mask]
        im = bg_img.astype(np.uint8)

        # logger.info(f'copy fg image {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
