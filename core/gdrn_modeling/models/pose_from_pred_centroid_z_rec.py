...
from core.utils.utils import allocentric_to_egocentric_gl, allocentric_to_egocentric_gl_torch, allo_to_ego_gl_mat_torch
...
def pose_from_pred_centroid_z_rec(
    pred_rots,
    pred_centroids,
    pred_z_vals,
    roi_cams,
    ...
    if is_train:
        return pose_from_predictions_train_rec(
            pred_rots,
            pred_centroids,
            pred_z_vals,
            roi_cams,
        ...
        return pose_from_predictions_test_rec(
            pred_rots,
            pred_centroids,
            pred_z_vals,
            roi_cams,
...
def pose_from_predictions_test_rec(
    pred_rots,
    pred_centroids,
    pred_z_vals,
    roi_cams,
    ...
    x_f = (cx - roi_cams[:, 0:1, 2]) / roi_cams[:, 0:1, 0]
    y_f = (cy - roi_cams[:, 1:2, 2]) / roi_cams[:, 1:2, 1]
    ...
    ty = (cy - py) * tz / fy
    """
    z = z / torch.sqrt(torch.square(x_f) + torch.square(y_f) + 1)
...
def pose_from_predictions_train_rec(
    pred_rots,
    pred_centroids,
    pred_z_vals,
    roi_cams,
    ...
    if pred_rots.ndim == 2 and pred_rots.shape[-1] == 4:
        ...
        quat_allo = pred_quats / (torch.norm(pred_quats, dim=1, keepdim=True) + eps)
        if is_allo:
            quat_ego = allocentric_to_egocentric_gl_torch(translation, quat_allo, eps=eps)
        else:
            quat_ego = quat_allo
        rot_ego = quat2mat_torch(quat_ego)
    if pred_rots.ndim == 3 and pred_rots.shape[-1] == 3:  # Nx3x3
        if is_allo:
            rot_ego = allo_to_ego_gl_mat_torch(translation, pred_rots, eps=eps)
        else:
            rot_ego = pred_rots
