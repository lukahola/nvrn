...
from core.utils.utils import CV2GL

from .engine_utils import batch_data, get_out_coor, get_out_mask, get_out_norm, cal_pose, ang_rectify
...
class GDRN_Evaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs, out_dict):
        ...
        cfg = self.cfg
        if cfg.TEST.USE_SVD:
            if cfg.TEST.SVD_TYPE.lower() == "all_once":
                return self.process_norm(inputs, outputs, out_dict)
            # elif cfg.TEST.SVD_TYPE.lower() == "net_iter_pnp":
            #     return self.process_net_and_pnp(inputs, outputs, out_dict, pnp_type="iter")
            # elif cfg.TEST.SVD_TYPE.lower() == "net_ransac_pnp":
    ...
    def process_net_and_pnp(self, inputs, outputs, out_dict, pnp_type="iter"):
        ...
        out_rots = out_dict["rot"].detach().to(self._cpu_device).numpy()
        out_transes = out_dict["trans"].detach().to(self._cpu_device).numpy()
        out_depth = out_dict["depth"].detach().to(self._cpu_device).numpy() # TODO: zty
    ...
    def process_norm(self, inputs, outputs, out_dict):
        """
        Args:
            inputs: the inputs to a model.
                It is a list of dict. Each dict corresponds to an image and
        ...
        out_norm_stc, out_norm_dyn = get_out_norm(cfg, out_norm_stc_x, out_norm_stc_y, out_norm_stc_z, out_norm_dyn_x, out_norm_dyn_y, out_norm_dyn_z)

        out_norm_stc = out_norm_stc.to(self._cpu_device).numpy()
        out_norm_dyn = out_norm_dyn.to(self._cpu_device).numpy()
        ...
        for i, (_input, output) in enumerate(zip(inputs, outputs)):
            ...
            for inst_i in range(len(_input["roi_img"])):
                ...
                # save normal
                path = osp.join(cfg.OUTPUT_DIR, 'normals')
                os.makedirs(path, exist_ok=True)
                if cfg.TEST.SAVE_NORMAL:
                    path = osp.join(path, f'{scene_id}-{im_id}-{obj_id}.pkl')
                    with open(path, 'wb') as f:
                        pickle.dump(np.concatenate([out_norm_stc[out_i], out_norm_dyn[out_i], out_mask[out_i]], axis=0), f)
                
                # get pose
                norm_stc_i = (out_norm_stc[out_i].transpose(1, 2, 0)-0.5)*2
                norm_dyn_i = (out_norm_dyn[out_i].transpose(1, 2, 0)-0.5)*2
                mask_i = np.squeeze(out_mask[out_i])

                _, rot_est = cal_pose(norm_stc_i, norm_dyn_i, mask_i, thresh=0.5, method='all_once')
                trans_est = out_transes[inst_i]
                rot_est = ang_rectify(trans_est, reverse=True).dot(rot_est)
                pose_est = np.hstack([rot_est, trans_est.reshape(3, 1)])
