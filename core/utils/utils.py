...
def allocentric_to_egocentric_gl(allo_pose_gl, src_type="mat", dst_type="mat", cam_ray=(0, 0, 1.0)):
    allo_pose_cv = allo_pose_gl.copy()
    allo_pose_cv[:3,:3] = CV2GL.T.dot(allo_pose_cv[:3,:3])
    ego_pose_cv = allocentric_to_egocentric(allo_pose_cv, src_type=src_type, dst_type=dst_type, cam_ray=cam_ray)
    ego_pose_gl = ego_pose_cv
    ego_pose_gl[:3,:3] = CV2GL.dot(ego_pose_gl[:3,:3])
    return ego_pose_gl
...
def egocentric_to_allocentric_gl(ego_pose_gl, src_type="mat", dst_type="mat", cam_ray=(0, 0, 1.0)):
    ego_pose_cv = ego_pose_gl.copy()
    ego_pose_cv[:3,:3] = CV2GL.T.dot(ego_pose_cv[:3,:3])
    allo_pose_cv = egocentric_to_allocentric(ego_pose_cv, src_type=src_type, dst_type=dst_type, cam_ray=cam_ray)
    allo_pose_gl = allo_pose_cv
    allo_pose_gl[:3,:3] = CV2GL.dot(allo_pose_gl[:3,:3])
    return allo_pose_gl


def egocentric_to_allocentric_norm(ego_pose, norm_dyn, src_type="mat", cam_ray=(0, 0, 1.0)):
    # Compute rotation between ray to object centroid and optical center ray
    cam_ray = np.asarray(cam_ray)
    if src_type == "mat":
        trans = ego_pose[:3, 3]
    ...
    if abs(angle) < 1e-6:
        ...
        rot_mat = axangle2mat(axis=np.cross(cam_ray, obj_ray), angle=-angle)
    shape = norm_dyn.shape
    # CV2GL
    norm_dyn = norm_dyn.reshape([-1,3])
    norm_dyn[:,1] = -norm_dyn[:,1]
    norm_dyn[:,2] = -norm_dyn[:,2]
    norm_dyn = rot_mat.dot(norm_dyn.T).T
    norm_dyn[:,1] = -norm_dyn[:,1]
    norm_dyn[:,2] = -norm_dyn[:,2]
    norm_dyn = norm_dyn.reshape(shape)
    # norm_dyn = CV2GL.dot(rot_mat).dot(CV2GL.T).dot(norm_dyn.reshape([-1,3]).T).T.reshape(shape)
    return norm_dyn
...
def allocentric_to_egocentric_gl_torch(translation, q_allo_gl, eps=1e-4):
    q_gl_to_cv = torch.zeros_like(q_allo_gl)
    q_gl_to_cv[:,1] = 1

    q_allo_cv = quatmul_torch(q_gl_to_cv, q_allo_gl)[:, 0]
    q_ego_cv = allocentric_to_egocentric_torch(translation, q_allo_cv, eps=eps)
    q_ego_gl = quatmul_torch(q_gl_to_cv, q_ego_cv)[:, 0]
...
def allo_to_ego_gl_mat_torch(translation, rot_allo_gl, eps=1e-4):
    rot_gl_to_cv = torch.zeros_like(rot_allo_gl)
    rot_gl_to_cv[:,0,0] = 1
    rot_gl_to_cv[:,1,1] = -1
    rot_gl_to_cv[:,2,2] = -1
    rot_allo_cv = torch.matmul(rot_gl_to_cv, rot_allo_gl)
    rot_ego_cv = allo_to_ego_mat_torch(translation, rot_allo_cv, eps=eps)
    rot_ego_gl = torch.matmul(rot_gl_to_cv, rot_ego_cv)
    return rot_ego_gl