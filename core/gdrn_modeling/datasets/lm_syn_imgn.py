...
class LM_SYN_IMGN_Dataset(object):
    ...
    def __init__(self, data_cfg):
        ...
        self.cache_dir = data_cfg.get("cache_dir", osp.join(PROJ_ROOT, ".cache"))  #TODO .cache
    ...
    def __call__(self):  # LM_SYN_IMGN_Dataset
        ...
        for ann_file, scene_root, xyz_root, norm_root in zip(tqdm(self.ann_files), self.image_prefixes, self.xyz_prefixes, self.norm_prefixes):
            ...
            for im_id in tqdm(sel_indices):
                ...
                stc_norm_path = osp.join(norm_root, f"{im_id}-normal-static.pkl")
                assert osp.exists(stc_norm_path), stc_norm_path
                dyn_norm_path = osp.join(norm_root, f"{im_id}-normal-dynamic-gl.pkl")
                ...
                inst = {
                    ...
                    "xyz_path": xyz_path,
                    "stc_norm_path": stc_norm_path,
                    "dyn_norm_path": dyn_norm_path,
                    "depth_render_path": depth_path, #TODO zty
                }
        ...
        mmcv.mkdir_or_exist(osp.dirname(cache_path)) # TODO zty
...
SPLITS_LM_IMGN_13 = dict(
    lm_imgn_13_train_1k_per_obj=dict(
        ...
        norm_prefixes=[osp.join(DATASETS_ROOT, "lm_imgn/norm_crop_imgn/") for _obj in LM_13_OBJECTS],
    ...
    for split in ["train"]:
        ...
        if name not in SPLITS_LM_IMGN_13:
            SPLITS_LM_IMGN_13[name] = dict(
                ...
                norm_prefixes=[osp.join(DATASETS_ROOT, "lm_imgn/norm_crop_imgn/")],
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
                with_depth=True,  # (load depth path here, but may not use it)
                depth_factor=1000.0,
