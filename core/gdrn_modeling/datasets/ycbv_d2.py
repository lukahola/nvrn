...
class YCBV_Dataset:
    ...
    def _load_from_idx_file(self, idx_file, image_root):
        ...
        norm_root = osp.join(image_root, "norm_crop")
        scene_gt_dicts = {}
        scene_gt_info_dicts = {}
        scene_cam_dicts = {}
        ...
        for (scene_id, im_id) in tqdm(scene_im_ids):
            ...
            for anno_i, anno in enumerate(anno_dict_list):
                ...
                if self.with_xyz:
                    ...
                    stc_norm_path = osp.join(norm_root, f"{scene_id:06d}/{im_id:06d}_{anno_i:06d}-normal-static.pkl")
                    assert osp.exists(stc_norm_path), stc_norm_path
                    inst["stc_norm_path"] = stc_norm_path

                    dyn_norm_path = osp.join(norm_root, f"{scene_id:06d}/{im_id:06d}_{anno_i:06d}-normal-dynamic-gl.pkl")
                    assert osp.exists(dyn_norm_path), dyn_norm_path
                    inst["dyn_norm_path"] = dyn_norm_path

                if self.with_depth:
                    depth_path = osp.join(image_root, f"{scene_id:06d}/depth/{im_id:06d}.png")
                    assert osp.exists(depth_path), depth_path
                    inst["depth_render_path"] = depth_path
    ...
    def models(self):
        ...
        cache_path = osp.join(self.cache_dir, "models_{}.pkl".format(self.name))
        if osp.exists(cache_path) and self.use_cache:
