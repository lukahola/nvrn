...
from core.utils.utils import CV2GL, egocentric_to_allocentric_gl, egocentric_to_allocentric_norm
...
class GDRN_DatasetFromList(Base_DatasetFromList):
    ...
    def read_data(self, dataset_dict):
        ...
        # logger.info(f'read image {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        if self.split == "train":
            ...
            if img_type == "syn":
                ...
                image, mask_trunc = self.replace_bg(image, mask, return_mask=True)
            else:  # real image
        ...
        # logger.info(f'change background {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        # logger.info(f'image augmentation {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        # logger.info(f'2d coord {time.perf_counter()-rtime}')
        rtime = time.perf_counter()

        #################################################################################
        if self.split != "train": # for test
            ...
            roi_keys = ["scene_im_id", "file_name", "cam", "im_H", "im_W",
                        "roi_img", "inst_id", "roi_coord_2d", "roi_cls", "score", "roi_extent",
                         ...
                         "scale", "resize_ratio", "model_info", "roi_depth", # zty
                        ]
            ...
            for inst_i, inst_infos in enumerate(dataset_dict["annotations"]):
                ...
                # roi_infos["depth_factor"].append(dataset_dict["depth_factor"][0])
                ...
                # roi_depth
                # depth = np.load(dataset_dict["depth_file"])
                depth = mmcv.imread(dataset_dict["depth_file"], "unchanged") #/ 1000.0 # TODO: mm -> m
                roi_infos["roi_depth"].append(depth.astype("float32"))
        ...
        # extent
        roi_extent = self._get_extents(dataset_name)[roi_cls]
        dataset_dict["roi_extent"] = torch.tensor(np.array(roi_extent), dtype=torch.float32)

        # logger.info(f'roi extent {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        # logger.info(f'load xyz {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        
        # load norm =======================================================
        dataset_dict["stc_norm_path"] = inst_infos["stc_norm_path"]
        stc_norm_info = mmcv.load(inst_infos["stc_norm_path"])
        # assert stc_norm_info['u'] == dyn_norm_info['u'], f'static normal: {inst_infos["stc_norm_path"]} / dynamic normal: {inst_infos["dyn_norm_path"]}'
        # assert stc_norm_info['l'] == dyn_norm_info['l'], f'static normal: {inst_infos["stc_norm_path"]} / dynamic normal: {inst_infos["dyn_norm_path"]}'
        # assert stc_norm_info['h'] == dyn_norm_info['h'], f'static normal: {inst_infos["stc_norm_path"]} / dynamic normal: {inst_infos["dyn_norm_path"]}'
        # assert stc_norm_info['w'] == dyn_norm_info['w'], f'static normal: {inst_infos["stc_norm_path"]} / dynamic normal: {inst_infos["dyn_norm_path"]}'
        u = stc_norm_info['u']
        l = stc_norm_info['l']
        h = stc_norm_info['h']
        w = stc_norm_info['w']
        norm_stc = np.zeros((im_H, im_W, 3)).astype(np.float32)
        norm_stc[u:(u+h),l:(l+w),:] = stc_norm_info['normal']

        dataset_dict["dyn_norm_path"] = inst_infos["dyn_norm_path"]
        dyn_norm_info = mmcv.load(inst_infos["dyn_norm_path"])
        u = dyn_norm_info['u']
        l = dyn_norm_info['l']
        h = dyn_norm_info['h']
        w = dyn_norm_info['w']
        norm_dyn = np.zeros((im_H, im_W, 3)).astype(np.float32)
        norm_dyn[u:(u+h),l:(l+w),:] = dyn_norm_info['normal']

        # load depth render================================================
        dataset_dict["depth_render_path"] = inst_infos["depth_render_path"]
        if inst_infos["depth_render_path"][-3:] == "npz":
            depth_render = np.load(inst_infos["depth_render_path"])["depth"]
            depth_render = np.expand_dims(depth_render, axis=2)
        elif inst_infos["depth_render_path"][-3:] == "png":
            depth_render = mmcv.imread(inst_infos["depth_render_path"], "unchanged")
            depth_render = np.expand_dims(depth_render, axis=2)

        depth_factor = dataset_dict["depth_factor"]

        # NOTE: full mask
        # mask_obj = ((norm_stc[:, :, 0] != 0) | (norm_stc[:, :, 1] != 0) | (norm_stc[:, :, 2] != 0) | (norm_dyn[:, :, 0] != 0) | (norm_dyn[:, :, 1] != 0) | (norm_dyn[:, :, 2] != 0)).astype(np.bool).astype(np.float32)
        if cfg.INPUT.SMOOTH_NORM:
            norm_stc = self.smooth_xyz(norm_stc)
            norm_dyn = self.smooth_xyz(norm_dyn)
        ...
        # logger.info(f'load norm {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        ## roi_norm ----------------------------------------------------
        roi_norm_stc = crop_resize_by_warp_affine(norm_stc, bbox_center, scale, out_res, interpolation=mask_xyz_interp)
        roi_norm_dyn = crop_resize_by_warp_affine(norm_dyn, bbox_center, scale, out_res, interpolation=mask_xyz_interp)
        pose = inst_infos["pose"]
        roi_norm_dyn = egocentric_to_allocentric_norm(pose, roi_norm_dyn)
        roi_norm = np.concatenate([roi_norm_stc, roi_norm_dyn], axis=-1)

        ## roi_depth ----------------------------------------------------zty
        # depth = np.load(dataset_dict["depth_file"])
        depth = mmcv.imread(dataset_dict["depth_file"], "unchanged") #/ dataset_dict["depth_factor"] # TODO: mm -> m
        roi_depth = np.zeros((1, im_H, im_W)).astype(np.float32)
        roi_depth = np.reshape(crop_resize_by_warp_affine(depth, bbox_center, scale, out_res, interpolation=mask_xyz_interp), [1, out_res, out_res])
        roi_depth = roi_depth * roi_mask_visib

        ## roi_depth_render----------------------------------------------zty
        depth_render = depth_render #/ dataset_dict["depth_factor"]
        roi_depth_render = crop_resize_by_warp_affine(depth_render, bbox_center, scale, out_res, interpolation=mask_xyz_interp)
        roi_depth_render = np.reshape(roi_depth_render, [1, out_res, out_res])

        # logger.info(f'crop resize warp {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        ...
        # logger.info(f'region {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        
        roi_norm = roi_norm.transpose(2, 0, 1)  # HWC-->CHW
        ...
        # normalize norm to [0, 1] using extent
        roi_norm = roi_norm / 2 + 0.5
        ...
        dataset_dict["roi_norm"] = torch.as_tensor(roi_norm.astype("float32")).contiguous()
        dataset_dict["roi_depth"] = torch.as_tensor(roi_depth.astype("float32")).contiguous() #TODO: zty
        dataset_dict["roi_depth_render"] = torch.as_tensor(roi_depth_render.astype("float32")).contiguous()
        # dataset_dict["depth_factor"] = torch.as_tensor(depth_factor).to(torch.float32)
        # pose targets ----------------------------------------------------------------------
        # pose = inst_infos["pose"]
        pose[:3,:3] = CV2GL.dot(pose[:3,:3])
        allo_pose = egocentric_to_allocentric_gl(pose)
        ...
        dataset_dict["allo_rot"] = torch.as_tensor(allo_pose[:3, :3].astype("float32"))
        ...
        if pnp_net_cfg.COORD_TYPE == 'rel-iso':
        ...
        if pnp_net_cfg.TRANS_TYPE == "centroid_z_rec":
            z_ratio = np.linalg.norm(inst_infos["trans"]) / resize_ratio
        else:
        ...
        # logger.info(f'prepare output {time.perf_counter()-rtime}')
        rtime = time.perf_counter()
        return dataset_dict
    ...
    def smooth_depth(self, depth):
        ...
        edges = get_edge(depth)
        depth[edges != 0] = depth_blur[edges != 0]
        depth[edges != 0] = depth_blur[edges != 0]
        return depth
